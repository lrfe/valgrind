...
/*--------------------------------------------------------------------*/
/*--- Platform-specific syscalls stuff.  syswrap-or1k-linux.c ----*/
/*--------------------------------------------------------------------*/
...
asm (
"   .text                                          \n"
"   .globl   do_syscall_clone_or1k_linux           \n"
"   do_syscall_clone_or1k_linux:                   \n\t"
"   l.addi r4, r4, -8                              \n\t"
"   l.sw   0(r4), r3                               \n\t"
"   l.sw   4(r4), r6                               \n\t"
...
static SysRes or1k_PRE_sys_mmap (ThreadId tid,
...
{
   VG_(threads)[tid].arch.vex.guest_r10 = tlsptr;
...
/* ---------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
#define PRE(name)       DEFN_PRE_TEMPLATE(or1k_linux, name)
#define POST(name)      DEFN_POST_TEMPLATE(or1k_linux, name)
...
/* Add prototypes for the wrappers declared here, so that gcc doesn't
   ...
   magic. */
DECL_TEMPLATE (or1k_linux, sys_mmap2);
DECL_TEMPLATE (or1k_linux, sys_rt_sigreturn);
DECL_TEMPLATE (or1k_linux, sys_set_thread_area);
DECL_TEMPLATE (or1k_linux, sys_ptrace);
DECL_TEMPLATE (or1k_linux, sys_unshare);
DECL_TEMPLATE (or1k_linux, sys_reboot);
DECL_TEMPLATE (or1k_linux, sys_sethostname);
...
/* ---------------------------------------------------------------------
   ...
   ------------------------------------------------------------------ */
#define PLAX_(sysno, name)    WRAPPER_ENTRY_X_(or1k_linux, sysno, name)
#define PLAXY(sysno, name)    WRAPPER_ENTRY_XY(or1k_linux, sysno, name)
...
static SyscallTableEntry syscall_main_table[] = {
   ...
   LINXY (__NR_clock_gettime64,        sys_clock_gettime64),
   LINX_ (__NR_clock_settime64,        sys_clock_settime64),
   LINXY (__NR_clock_getres_time64,    sys_clock_getres_time64),
   LINXY (__NR_clock_nanosleep_time64, sys_clock_nanosleep_time64),
   LINXY (__NR_timer_gettime64,        sys_timer_gettime64),
   LINXY (__NR_timer_settime64,        sys_timer_settime64),
   LINXY (__NR_timerfd_gettime64,      sys_timerfd_gettime64),
   LINXY (__NR_timerfd_settime64,      sys_timerfd_settime64),
   LINX_ (__NR_utimensat_time64,       sys_utimensat_time64),
   LINXY (__NR_pselect6_time64,        sys_pselect6_time64),
   LINXY (__NR_ppoll_time64,           sys_ppoll_time64),
   LINXY (__NR_recvmmsg_time64,        sys_recvmmsg_time64),
   LINX_ (__NR_mq_timedsend_time64,    sys_mq_timedsend_time64),
   LINXY (__NR_mq_timedreceive_time64, sys_mq_timedreceive_time64),
   LINX_ (__NR_semtimedop_time64,      sys_semtimedop_time64),
   LINXY (__NR_rt_sigtimedwait_time64, sys_rt_sigtimedwait_time64),
   LINXY (__NR_futex_time64,           sys_futex_time64),
...
#endif // defined(VGP_or1k_linux)

/*--------------------------------------------------------------------*/
/*--- end                                 syswrap-or1k-linux.c ---*/
/*--------------------------------------------------------------------*/
